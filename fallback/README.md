# fallback — the previous system, kept runnable

This folder is **reconizer**, the system we drove before the current pilot in
[`../main/`](../main/). It is here on purpose, and it is not
...
└── firmware/esp32/    the SAME firmware as main/ (see below)
```
...
`fallback/firmware/esp32/` and `main/firmware/esp32/` hold the **same**
...
- **The Python side of the button is not ported.** `botones.py` and the protocol
  changes that read the button bits exist only in `main/`. Reconizer is
