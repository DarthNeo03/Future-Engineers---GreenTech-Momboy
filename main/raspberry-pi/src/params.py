...
ESQUEMA: Dict[str, Dict[str, Dict[str, Any]]] = {
    ...
    "geometria": {
        ...
        "fx_px": _p("float", 460.0, "Focal horizontal en pixeles (referida a 640 de ancho). Se calibra con un clic sobre un objeto desplazado a un lado. Con fx_auto encendido no se usa.", 100.0, 2000.0),
        "fx_auto": _p("bool", False, "PIXELES CUADRADOS: tomar la focal horizontal de la vertical en vez de fx_px. fx_px va referido a 640 de ancho y fy_px a 480 de alto, y cada uno se escala con SU lado de la captura: mientras se capture a 640x480 dan lo mismo, pero en cuanto la camara entrega otra cosa (se pide 1920x480 y da 1280x720, que es lo que hace esta) se separan un 33 %, los milimetros LATERALES salen un 25 % cortos y el carro cree que la pared esta mas cerca y que el pasillo es mas estrecho de lo que son. En el Open Challenge apenas se nota (la distancia al muro sale de las filas, o sea de fy); en el reto de obstaculos decide por donde se pasa cada pilar y es la causa de 'no se atreve a pasar aunque tiene sitio de sobra'. Enciendelo salvo que tengas una lente anamorfica; al calibrar fx a mano desde la web se apaga solo."),
        ...
        "radio_giro_mm": _p("float", 550.0, "RADIO DE GIRO del carro con el volante a tope (dir = 100), en mm, medido al eje del carro. Es lo que convierte la curvatura que pide el esquive en % de volante (modo 'arco'): sin esto el esquive pide la mitad o el doble de lo que hace falta. Para medirlo: carro en manual con direccion a tope, una vuelta completa despacio, y el DIAMETRO del circulo que dibuja el centro del carro entre dos. Tipico entre 450 y 800.", 200.0, 2000.0),
    },
    ...
    "obstaculos": {
        ...
        "activar_desde_mm": _p("float", 1500.0, "Distancia a la que un pilar empieza a influir en la direccion.", 300.0, 4000.0),
        "mandar_desde_mm": _p("float", 1100.0, "Distancia a la que el pilar ya manda al maximo sobre la direccion (y el centrado se calla). Cambiar de carril 20 cm necesita ~1 m de recta con el radio de giro tipico: por debajo de 1000 el carro llega al pilar sin haber terminado de apartarse.", 100.0, 2000.0),
        "margen_mm": _p("float", 70.0, "Holgura extra entre el costado del carro y el pilar al pasarlo.", 0.0, 300.0),
        "margen_pared_mm": _p("float", 30.0, "Holgura entre el costado del carro y la PARED cuando el punto de paso se recorta al pasillo libre. Menos que margen_mm a proposito: con el pilar a 30 cm de la pared y un carro de 20 hay 7 cm que repartir, y rozar la pared no penaliza (mover el pilar, si).", 0.0, 150.0),
        ...
        "dir_max_pct": _p("float", 80.0, "TOPE de direccion que puede pedir el esquive. En modo 'arco' el volante pedido sale de la geometria (radio_giro_mm) y llega a tope solo cuando un pilar aparece muy cerca y dead-ahead, que es cuando de verdad hace falta; 80 deja un margen contra el derrape. En 'punto' y 'borde' era el freno contra el volantazo: si con esos modos necesitas mas, acercate mas tarde (mandar_desde_mm) en vez de subir esto.", 10.0, 100.0),
        "mirada_min_mm": _p("float", 350.0, "Mirada minima. En 'arco' es a lo que se acorta la mirada cuando el carro ya esta casi sobre la linea de paso (mas corta = converge antes, pero puede serpentear). En 'punto' es lo que impide el volantazo al acercarse: sin ella el mismo desvio lateral pide cada vez mas angulo hasta llegar al tope.", 150.0, 1200.0),
        ...
        "peso_max": _p("float", 1.0, "Peso maximo del esquive frente al centrado. 1 = dentro de mandar_desde_mm el pilar manda del todo y el centrado se calla. HACE FALTA QUE SEA 1: el centrado del carril lleva kp de cientos y con 0,8 su 20 % restaba mas de la mitad de lo que pedia el pilar (el carro se apartaba un poco y se llevaba el pilar con el costado). El muro sigue contando por su lado: el punto de paso se recorta al pasillo libre y un pilar encima dispara el escape.", 0.0, 1.0),
        "modo": _p("str", "arco", "Como se esquiva. 'arco' (recomendado): pure pursuit hacia la LINEA de paso paralela al carril; el volante sale de la curvatura del arco y del radio de giro real del carro (geometria.radio_giro_mm), asi que pide lo que la fisica pide. 'punto': se apunta al hueco con una ganancia fija (k_dir) y una mirada minima; pedia la mitad de lo necesario de cerca. 'borde': se mantiene el pilar pegado al CANTO de la imagen y, al tenerlo encima, se da el giro grande; control visual puro, no depende de las distancias.", opciones=["arco", "punto", "borde"]),
        "k_arco": _p("float", 1.0, "(modo 'arco') ajuste fino sobre el volante que sale de la geometria. 1 = geometria pura. Subelo si el carro se queda corto al apartarse (tipico si radio_giro_mm esta medido de menos o el servo no llega a tope); bajalo si se abre de mas.", 0.3, 3.0),
        "mirada_mm": _p("float", 600.0, "(modo 'arco') mirada MAXIMA sobre la linea de paso, en mm por delante. Nunca mas alla del pilar, y se acorta hasta mirada_min_mm cuando queda poco que desplazarse (asi los ultimos centimetros no tardan un metro en llegar). Mas corta = cambio de carril mas brusco y antes; mas larga = mas suave pero necesita mas recta. Con 600 un desplazamiento de 20 cm pide ~60 % de volante y cruza el carro unos 20 grados.", 200.0, 1500.0),
        "vel_esquive": _p("int", 40, "Velocidad (% de vmax) a la que se baja mientras un pilar manda o queda al costado. A la de crucero no hay volante que desplace el carro 20 cm en los ultimos 70 cm: VER UN PILAR ES FRENAR. Se aplica en proporcion al peso del esquive, asi que de lejos apenas frena.", 10, 100),
        "desvio_max_esquive_deg": _p("float", 35.0, "Cuanto se deja cruzar el carro (grados respecto al rumbo de la recta, por giroscopio) para apartarse de un pilar. Pasado esto no se abre mas, solo se endereza: un carro 40 grados torcido en un carril de 60-100 cm se mete en la pared de al lado antes de llegar al pilar.", 10.0, 80.0),
        "pilar_parar_mm": _p("float", 180.0, "Si un pilar sigue DENTRO del corredor de las ruedas a menos de esto, el esquive no lo ha librado: el navegador hace un escape marcha atras (volante al lado contrario al de paso, para que el morro quede apuntando al hueco) en vez de chocar. Es la red: si salta a menudo, el esquive empieza tarde (activar/mandar_desde_mm) o va rapido (vel_esquive).", 60.0, 500.0),
        "pilar_salir_mm": _p("float", 500.0, "Hasta que distancia se retrocede en ese escape si el pilar sigue en el corredor. Da sitio para que el arco lo libre al volver a avanzar.", 200.0, 1200.0),
        "pilar_escape_dir_pct": _p("float", 45.0, "Volante durante la reversa del escape por pilar, en %. Va al lado CONTRARIO al de paso (con Ackermann eso hace girar el morro hacia el hueco). Menos que el escape por muro a proposito: con el volante a tope el carro rota 30 grados en un segundo y acaba de lado en el carril.", 0.0, 100.0),
        "magenta_semi_mm": _p("float", 110.0, "Medio ancho con el que se rodea un delimitador MAGENTA del cajon. Mide 200x20x100 y se ve de canto (20 mm en la imagen), pero el despeje hay que contarlo desde su extremo: medio muro de 200 mas un poco. Los pilares usan semi_pilar_mm.", 25.0, 250.0),
        "no_volver_al_costado": _p("bool", True, "Mientras el pilar esta AL COSTADO (ya paso el morro, todavia no la cola) se LIMITA el volante hacia el. Con direccion Ackermann volver al centro del carril con el pilar a la altura de la rueda trasera es barrerlo. Se sigue por estima con la velocidad y el giroscopio; sin giroscopio manda el compromiso por tiempo (y ahi no se gira nada hacia el)."),
        "costado_giro_max_pct": _p("float", 25.0, "Tope de volante HACIA el pilar que esta al costado, en %. El tope real es menor: la mitad de la holgura en mm con la que se le esta pasando (5 cm de aire = 25 %, 2 cm = 10 %). Enderezar el rumbo hace falta permitirlo, o el carro sigue cruzado hacia la pared de al lado durante todo el paso.", 0.0, 60.0),
        "pilares_no_son_muro": _p("bool", True, "UN PILAR NO ES UNA PARED. El perfil del muro busca donde deja de verse el piso; con el metodo 'piso' el pilar entero cuenta como muro y con el metodo 'negro' cuenta SU SOMBRA sobre el tapete blanco, que cae justo en su base. En los dos casos el perfil pone una pared de 5 cm donde hay una señal: el pasillo se cierra, el carro frena, cree ver una esquina donde no la hay y hasta retrocede — justo cuando lo que tiene que hacer es rodearla. Con esto, las cajas de los pilares detectados se borran del muro (mas sombra_pilar_px por debajo) y el perfil vuelve a describir solo las PAREDES. De los pilares se encarga el esquive, que para eso los sigue uno a uno. Solo actua con obstaculos.activo."),
        "sombra_pilar_px": _p("int", 8, "Pixeles que se borran del muro POR DEBAJO de la base del pilar, donde cae su sombra sobre el tapete. Subelo si en la vista 'piso' se sigue viendo un muro pegado a los pilares; bajalo si se come el contacto de la pared que hay detras.", 0, 40),
        "pasillo_min_maniobra_mm": _p("float", 80.0, "Mientras se esquiva, el pasillo crudo NO frena ni dispara la reversa (el carro va cruzado a proposito y ese pasillo mide la pared de al lado, no un muro de frente): manda la pared de FRENTE. Este es el suelo de seguridad que se mantiene igualmente — por debajo de esto hay algo pegado al morro. Tiene que ser BAJO: el corredor de las ruedas lleva margen_ruedas_mm de propina, asi que una pared que queda 5 cm libre del carro de verdad ya asoma en el pasillo a 15 cm, y con 150 aqui el carro se metia en reversa por una pared que no iba a tocar. Con el pilar AL COSTADO no hay suelo ninguno: lo que el pasillo mide entonces es el propio pilar.", 0.0, 400.0),
        "costado_desde_mm": _p("float", 120.0, "Desde cuantos mm antes del morro el pilar cuenta como AL COSTADO: ahi el volante ya no cambia por donde se pasa, solo cuanto barre el costado, asi que manda el tope hacia el pilar y no el arco. Mas o menos donde la camara deja de verlo por abajo.", 0.0, 400.0),
        "recuperar_ms": _p("int", 1000, "Cuanto dura la SALIDA del paso desde que la cola deja atras el pilar. El carro queda pegado a una pared y algo cruzado: durante esto se sigue a vel_esquive y el navegador sigue midiendo el muro de frente por su orientacion (no por el pasillo crudo, que con el carro cruzado lee la pared de al lado y disparaba una reversa nada mas pasar el pilar).", 0, 4000),
        ...
        "verificar_tamano": _p("str", "suave", "LA PRUEBA FISICA. Una señal mide 50x50x100 mm (regla 13.1): sabiendo a que distancia esta, la geometria dice cuanto TIENE que medir en pixeles, y lo que no cuadra no es un pilar (sombras rojizas del zocalo, reflejos, trozos de pared). El peligro es que una prueba de tamaño mal calibrada no se equivoca poco: BORRA TODOS LOS PILARES y deja al carro ciego. Por eso hay tres niveles. 'suave' (por defecto) solo comprueba el ALTO, que depende de fy — el parametro fiable, porque la altura de captura es la misma con la que se calibro — y del ancho solo mata lo imposible; separar el pilar del delimitador magenta queda entonces en manos del veto por solapamiento, que no usa geometria. 'estricto' añade la banda de ancho real en mm: caza al delimitador aunque el magenta este mal calibrado, pero necesita fx BIEN medido (ojo: fx se escala con el ANCHO de captura, asi que grabando a 1920 con fx calibrado a 640 sale 3 veces mayor y un pilar de 50 mm se mide como 17). 'no' apaga la prueba. Comprueba con tools/diagnostico_pilares.py cuanto mide un pilar de verdad antes de subir a 'estricto'.", opciones=["no", "suave", "estricto"]),
        ...
        "pilar_alto_tol": _p("float", 0.6, "Tolerancia del alto esperado, en fraccion. 0.6 = se acepta entre el 40 % y el 160 % de lo que dice la geometria. Generosa a proposito: la base del pilar se ve mal de lejos y el contraste se come filas. Los objetos recortados por el canto de la imagen no pasan esta prueba, se dan por buenos. Si el diagnostico dice que se estan cayendo pilares buenos por el alto, subela antes que apagar la prueba entera.", 0.1, 0.95),

        ### SEGUIR EL PILAR AUNQUE LA MASCARA PARPADEE
        ### El agujero por el que se escapaba el lado. El detector de color se
        ### cae unos frames -brillo del tapete, exposicion automatica, el pilar
        ### entrando en el punto ciego- y el esquive se iba a "recto": el carro
        ### se olvidaba del lado y adelantaba por donde tocara. Se ve clavado en
        ### el video: el pilar ahi delante y el rotulo diciendo ADELANTANDO.
        "seguir_a_ciegas": _p("bool", True, "Cuando el pilar deja de verse un par de frames, SEGUIR conduciendo contra su ultima posicion conocida en vez de soltar el volante. La posicion se adelanta cada frame con la velocidad real del carro. Es lo que mantiene el lado de paso a traves de los parpadeos del detector; sin esto, cada hueco en la mascara se traduce en 'ir recto' y el pilar se pasa por donde toque. Apagalo solo para comprobar cuanto parpadea de verdad tu deteccion."),
        "ciego_max_ms": _p("int", 600, "Cuanto se conduce contra la posicion estimada sin volver a ver el pilar. Pasado esto manda el compromiso de adelantamiento (ir recto). Subelo si en el video ves que el pilar desaparece mucho rato y el carro se endereza antes de tiempo; bajalo si el carro persigue pilares que ya no estan.", 0, 3000),
        "ciego_vistas_min": _p("int", 2, "Cuantas veces hay que haber visto un pilar antes de aceptarle que se conduzca a ciegas contra el. Con 1, un destello suelto de color en una pared puede llevarse al carro medio segundo hacia un fantasma.", 1, 10),
        "peso_ciego": _p("float", 0.9, "Cuanto pesa el esquive mientras se conduce contra la posicion ESTIMADA, como fraccion del peso normal. Algo menos que verlo de verdad, pero muchisimo mas que olvidarse de el.", 0.0, 1.0),
