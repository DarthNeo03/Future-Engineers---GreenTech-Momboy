#!/usr/bin/env python3
"""
main.py — Piloto WRO 2026, RETO DE OBSTACULOS.

Lo mismo que el Open Challenge (main/open-challenge/) mas dos cosas:
  * ESQUIVAR PILARES: el ROJO se pasa por su DERECHA y el VERDE por su
    IZQUIERDA, y esos lados no se invierten con el sentido de la ronda.
  * RESCATE DE ESQUINA: cuando el piso queda en TRIANGULO -el carro metido en
    el rincon, con las dos paredes cerrando- se da un giro comprometido hacia
    adentro, porque ahi la reversa del escape no saca al carro.
...
El carro ARRANCA DESARMADO. En competencia se arma con el PULSADOR que cuelga
del ESP32 (una pulsacion arranca la ronda, otra la corta); desde la web el
boton ARMAR hace lo mismo.
"""
