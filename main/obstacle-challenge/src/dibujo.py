...
from .navegacion import Decision, RECTO, PRE_GIRO, GIRO, ESCAPE, RESCATE
...
def anotar(frame: np.ndarray,
           ...
           hud: Dict[str, Any],
           obst: Optional[Dict[str, Any]] = None,
           atrapado: Optional[Dict[str, Any]] = None) -> np.ndarray:
    H, W = frame.shape[:2]
    ...
    if perfil is not None:
        ...
        # Piso por mitades: la barra que gana es por donde se sale de un
        # rincon. Va aparte de las de espacio libre a proposito, porque en la
        # esquina aquellas empatan y esta no.
        barra(30, 46, perfil.piso_izq, f"p{perfil.piso_izq:.2f}", (230, 230, 230))
        barra(W - 46, W - 30, perfil.piso_der, f"p{perfil.piso_der:.2f}",
              (230, 230, 230))
    ...
    # los umbrales con el carro puesto a mano en el rincon.
    if atrapado and "piso_pct" in atrapado:
        col = ((0, 0, 255) if atrapado.get("atrapado") else
               (0, 200, 255) if atrapado.get("patron") else (150, 150, 150))
        cv2.putText(frame,
                    f"triangulo: piso {atrapado['piso_pct']}% "
                    f"relieve {atrapado['relieve_pct']}% "
                    f"lejos {atrapado['lejos_mm']}mm "
                    f"{atrapado['sostenido_ms']}ms",
                    (6, H - 104), cv2.FONT_HERSHEY_SIMPLEX, 0.38, col, 1,
                    cv2.LINE_AA)
        if atrapado.get("fallo"):
            cv2.putText(frame, atrapado["fallo"], (6, H - 118),
