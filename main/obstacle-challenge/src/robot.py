...
En competencia el start no se da desde la web sino con el PULSADOR, que cuelga
...
from . import botones as botones_mod
from . import camara as cam_mod
from . import color_config as cc
from . import dibujo, lineas as lineas_mod, muro, params as params_mod, vision
from . import protocolo as P
from .carrera import Carrera, LISTO, TERMINADO
...
class Robot:
    ...
    def armar(self, si: bool) -> None:
        ...
        if si:
            ...
            self.detector_atrapado.reiniciar()
            if self.modo == "auto":
    ...
    # A proposito NO pasa por el mismo camino que el boton ARMAR de la web:
    # un solo pulsador tiene que poder arrancar Y parar, y lo que se quiera
    # hacer desde la web no puede deshacerse desde la pista, ni al reves.
    ...
    def forzar_rescate(self) -> None:
        """Dispara el rescate de esquina a mano (boton de la web), para verlo
        en el banco sin tener que meter el carro en un rincon de verdad."""
        self._forzar_rescate = True
        self.log("[prueba] rescate de esquina forzado")
    ...
    def muestrear_tcs(self, que: str) -> Dict[str, Any]:
        ...
        for k, v in nuevos.items():
            # Muestrear una LINEA solo puede BAJAR el suelo de luz. Si lo
            # subiera, muestrear el naranja (que devuelve bastante claro)
            # dejaria fuera a la azul, que es mucho mas oscura: exactamente el
            # fallo que hizo que el TCS no viera nunca la azul.
            if k == "c_min" and que != "blanco" and v >= int(self.p["tcs"]["c_min"]):
                continue
    ...
    def _bucle(self):
        ...
        while not self._parar.is_set():
            ...
            # --- vision ---------------------------------------------------
            masks = self.vision.solo_mascaras(hsv, ["blanco", "negro"])
            dets, masks_det = self.vision.detectar_en(
                hsv, ["naranja", "azul", "rojo", "verde"])
            ...
            if time.time() - self.lineas.t_ultimo_evento < 0.1:
                self.t_linea_reciente = time.time()
            ...
            if perfil is not None:
                atrapado = self.detector_atrapado.paso(perfil, self.p["rescate"])
            if self._forzar_rescate:
                self._forzar_rescate = False
                atrapado = True
            ...
            try:
                dibujo.anotar(anotado, perfil, d, self.geo, dets,
                              self.perfil_color["colores"],
                              self.carrera.estado_dict(), hud,
                              self.esquivador.info,
                              self.detector_atrapado.info)
            except Exception as e:
                self.log(f"[dibujo] {e}")
    ...
    def estado(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "atrapado": self.detector_atrapado.info,
